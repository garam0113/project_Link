...
VALUES (5, 'user555', 22, '모임원 추가', '실시간 계좌이체', 5000, SYSDATE, 0, 10, 'a' );
		...
		TO_CHAR(CP.CLUB_POST_REG_DATE, '""YYYY"년 "MM"월 "DD"일" ""HH24"시 "MI"분 "SS"초"') AS CLUB_POST_REG_DATE,
		TO_CHAR(CP.CLUB_POST_UPDATE_DATE, '""YYYY"년 "MM"월 "DD"일" ""HH24"시 "MI"분 "SS"초"') AS CLUB_POST_UPDATE_DATE
		FROM CLUB_POST CP, USERS U, ( SELECT SOURCE_NO AS HEART_CONDITION
												FROM HEART
												WHERE TRIM(SOURCE) = '2' AND USER_ID = 'user333' ) V1
		WHERE CP.USER_ID = U.USER_ID AND CP.CLUB_POST_NO = HEART_CONDITION (+) AND CP.USER_ID = 'user333' AND CP.REPORT_CONDITION = '0' AND CP.DELETE_CONDITION = '0'
		ORDER BY CP.CLUB_POST_REG_DATE DESC
