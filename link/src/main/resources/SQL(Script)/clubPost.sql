...
SELECT H.SOURCE_NO, H.USER_ID
FROM HEART H, ( SELECT C.CLUB_POST_COMMENT_NO, C.CLUB_POST_NO, C.USER_ID, U.NICKNAME, U.PROFILE_IMAGE, C.COMMENT_CONTENT, C.COMMENT_REG_DATE, C.COMMENT_UPDATE_DATE, C.COMMENT_HEART_COUNT, C.COMMENT_COUNT, C.REPORT_CONDITION, C.DELETE_CONDITION, C.PARENT, C.DEPTH, C.SEQUENCE
				FROM CLUB_POST_COMMENT C, USERS U
				WHERE C.USER_ID = U.USER_ID AND C.CLUB_POST_NO = 75 AND C.DELETE_CONDITION = 0 AND C.PARENT = 
				ORDER BY C.CLUB_POST_COMMENT_NO ASC ) V4
WHERE H.SOURCE_NO = V4.CLUB_POST_COMMENT_NO AND H.USER_ID = 'user03' AND H.SOURCE = 3
