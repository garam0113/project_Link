...
FROM ( SELECT ROW_NUMBER() OVER(ORDER BY CLUB_POST_NO DESC) AS R, V2.*
		FROM ( SELECT ROW_NUMBER() OVER(ORDER BY CLUB_POST_NO DESC ) AS R, V3.*
				FROM ( SELECT CP.CLUB_NO, CP.CLUB_POST_NO, CP.IMAGE1, CP.CLUB_POST_VIDEO1, CP.CLUB_POST_TITLE, CP.USER_ID, U.PROFILE_IMAGE, U.NICKNAME, CP.CLUB_POST_HEART_COUNT
						FROM CLUB_POST CP, USERS U
						WHERE CLUB_NO = 2 AND REPORT_CONDITION = '0' AND DELETE_CONDITION = '0' AND CP.USER_ID = U.USER_ID ) V3 ) V2 ) V3
